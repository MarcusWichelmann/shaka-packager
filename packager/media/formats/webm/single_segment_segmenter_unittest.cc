...
TEST_F(SingleSegmentSegmenterTest, BasicSupport) {
  ...
  ASSERT_OK(segmenter_->FinalizeSegment(0, 5 * kDuration, !kSubsegment, false));
...
TEST_F(SingleSegmentSegmenterTest, SplitsClustersOnSegment) {
  ...
  ASSERT_OK(
      segmenter_->FinalizeSegment(5 * kDuration, 8 * kDuration, !kSubsegment, false));
  ASSERT_OK(segmenter_->Finalize());
...
TEST_F(SingleSegmentSegmenterTest, IgnoresSubsegment) {
  ...
  ASSERT_OK(segmenter_->FinalizeSegment(0, 8 * kDuration, !kSubsegment, false));
  ASSERT_OK(segmenter_->Finalize());
